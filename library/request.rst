...
The above types of request are useful because they are asynchonous.  That is, the user can continue to view and interact with the original page while additional data is being requested.  A more common way to request data asynchronously is to use the `XMLHttpRequst <http://www.w3.org/TR/XMLHttpRequest/>`_ object.  With an XMLHttpRequest object, you can open a conection to a server and send or receive data via HTTP (or HTTPS).  XMLHttpRequest objects are a bit awkward to use and are unfortunately not supported in all browsers.  OpenLayers 2 provides a cross-browser `XMLHttpRequest <http://code.google.com/p/xmlhttprequest/>`_ function and wraps it in some convenient ``OpenLayers.Request`` methods.
